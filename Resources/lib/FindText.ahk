...
FindText_Gui(cmd, arg1:="")
{
  ...
  if (cmd="MakeCaptureWindow")
  {
    ...
    Gui, Add, Button, xm vBind0 gFindText_Run, BindWindow1
    Gui, Add, Button, x+15 vBind1 gFindText_Run, BindWindow1+
    Gui, Add, Button, x+15 vBind2 gFindText_Run, BindWindow2
    Gui, Add, Button, x+15 vBind3 gFindText_Run, BindWindow2+
    Gui, Add, Button, x+15 vBind4 gFindText_Run, BindWindow3
  ...
  if (cmd="MakeMainWindow")
  {
    ...
    Gui, Add, Button, xm y+18 w144 vCutL2 gFindText_Run, CutL
    Gui, Add, Button, x+0 wp vCutR2 gFindText_Run, CutR
    Gui, Add, Button, x+0 wp vCutU2 gFindText_Run, CutU
    Gui, Add, Button, x+0 wp vCutD2 gFindText_Run, CutD
    Gui, Add, Button, x+0 wp vUpdate gFindText_Run, Update
    Gui, Font, s6 bold, Verdana
    Gui, Add, Edit, xm y+10 w720 r20 vMyPic -Wrap
    Gui, Font, s12 norm, Verdana
    Gui, Add, Button, xm w240 vCapture gFindText_Run, Capture
  ...
  if (cmd="Capture"||cmd="CaptureS")
  {
    ...
    if (ShowScreenShot)
    {
      ...
      {
        WinGetPos, x, y
        FindText.GetBitsFromScreen(0,0,0,0,0,x1,y1)
    ...
    if (cors.bind!="")
    {
      ...
      Result:="`nSetTitleMatchMode, 2`nid:=WinExist(""" tt """)"
        ...
        . ")  `; Unbind Window using FindText.BindWindow(0)`n`n" Result
    }
    if (Event="ButtonOK")
    {
      if (!A_IsCompiled)
  ...
  {
    ListLines, %lls%
    local bits, zx, zy, zw, zh, win, hDC, mDC, hBM, oBM, hBrush, oBrush
    Gui, FindText_ScreenShot: Destroy
    bits:=FindText.GetBitsFromScreen(0,0,0,0,0,zx,zy,zw,zh)
    if (!arg1 or !bits.Scan0 or zw<1 or zh<1)
      return
  ...
  if (cmd="getcors")
  {
    ...
    Loop, % 2*hh+1
    {
      j:=py-hh+A_Index-1
      Loop, % 2*ww+1
        i:=px-ww+A_Index-1, cors[++k]:=FindText.GetColor(i,j,0)
    }
    ListLines, %lls%
  ...
  if (cmd="GetRange")
  {
    ...
    FindText.GetBitsFromScreen(0,0,0,0,0,x,y,w,h)
  ...
  if (cmd="Test"||cmd="TestClip")
  {
    ...
    if (!A_IsCompiled) and InStr(s,"MCode(") and (cmd="Test")
    {
      ...
      pid:=FindText_Gui("Exec", s)
      DetectHiddenWindows, On
      WinWait, ahk_class AutoHotkey ahk_pid %pid%,, 3
      if (!ErrorLevel)
        WinWaitClose,,, 30
    }
    else
  ...
  if (cmd="Exec")
  {
    ...
    pid:="", s:=StrReplace(arg1, "`r")
    s:="DllCall(""SetWindowText"",Ptr,A_ScriptHwnd,Str,""<AHK>"")`n" s
    Try
    {
      local oExec
      oExec:=ComObjCreate("WScript.Shell").Exec("""" Ahk """ /f *")
      oExec.StdIn.Write(s)
  ...
  if (cmd="Bind0"||cmd="Bind1"||cmd="Bind2"||cmd="Bind3"||cmd="Bind4")
  {
    ListLines, %lls%
    Critical
  ...
  if (cmd="ButtonOK"||cmd="SplitAdd"||cmd="AllAdd")
  {
    ...
    wz:=Format("{:d}",InStr(wz,"`n")-1) "." FindText.bit2base64(wz)
  ...
  if (cmd="CutL2"||cmd="CutR2"||cmd="CutU2"||cmd="CutD2")
  {
    ListLines, %lls%
    Gui, FindText_Main: Default
    GuiControlGet, s,, MyPic
    s:=Trim(s,"`n") . "`n", k:=SubStr(cmd,4,1)
    if (k="U")
      s:=RegExReplace(s,"^[^\n]+\n")
    else if (k="D")
      s:=RegExReplace(s,"[^\n]+\n$")
  ...
  if (cmd="Update")
  {
    ...
    k:="{End}{Shift Down}{Home}{Shift Up}{Del}"
    ControlSend,, %k%, ahk_id %hscr%
...
CutU2      = Cut the Upper Edge of the text in the edit box below
CutL2      = Cut the Left Edge of the text in the edit box below
CutR2      = Cut the Right Edge of the text in the edit box below
CutD2      = Cut the Lower Edge of the text in the edit box below
Update     = Update the text in the edit box below to the line of code
...
Close      = Close the Window Dont Do Anything
...
Bind0      = Bind the window and Use GetDCEx() to get the image of background window
Bind1      = Bind the window Use GetDCEx() and Modify the window to support transparency
Bind2      = Bind the window and Use PrintWindow() to get the image of background window
Bind3      = Bind the window Use PrintWindow() and Modify the window to support transparency
Bind4      = Bind the window and Use PrintWindow(,,3) to get the image of background window
)
    return s
  }
  ListLines, %lls%
  ...
  , ByRef zx:="", ByRef zy:="", ByRef zw:="", ByRef zh:="")
{
  local
  global FindText
  static Ptr:="Ptr", init:=!FindText.GetBitsFromScreen(0,0,0,0,1)
  bits:=this.bits
  if (!ScreenShot)
  {
  ...
  if (zw<1 or zh<1)
    this.xywh2xywh(x,y,w,h,x,y,w,h,zx,zy,zw,zh)
  if (zw>bits.oldw or zh>bits.oldh or !bits.hBM)
  {
    if (bits.hBM)
    ...
    bits.oldw:=zw, bits.oldh:=zh, x:=zx, y:=zy, w:=zw, h:=zh
  }
  ...
  , ByRef input, num, ByRef allpos, allpos_max)
{
  ...
  return !bits.Scan0 ? 0:DllCall(&MyFunc, "int",mode, "uint",color
...
{
  local
  ocr_Text:=ocr_X:=ocr_Y:=min_X:="", this.ScreenShot()
  For k,v in ok
    x:=v.1
    ...
    For k,v in ok
    {
      ...
      else if (x=LeftX)
      {
        Loop, 100
        {
          ...
          if this.FindText(LeftX,LeftY,LeftX+LeftW-1,LeftY+LeftH-1,err,err,Text,0)
            Break
          if this.FindText(x, y, x+w-1, y+h-1, err, err, Text, 0)
          {
            LeftX:=x, LeftY:=y, LeftW:=w, LeftH:=h, LeftOCR:=v.id
            Break
          }
  ...
  , ScreenShot:=1, ByRef rx:="", ByRef ry:="")
{
  ...
  rx:=x+1+w//2, ry:=y+1+CutUp+(h-CutUp-CutDown)//2
